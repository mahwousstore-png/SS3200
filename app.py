...
def build_simple_salla_html(name: str, d: dict, store_name: str, store_link: str) -> str:
    ...
    a_tag = f'<a href="{store_link}" style="color:#d4af37;font-weight:bold;text-decoration:none;">{store_name}</a>' if store_name else f'<strong style="color:#d4af37;">{store_name}</strong>'
...
async def fetch_notes_async(session, name: str, api_key: str, model: str, store_name: str, semaphore):
    system_msg = """أنت خبير محتوى وتسويق عطور. أرجع وصفاً دقيقاً وطويلاً كـ JSON فقط وبدون أي إضافات:
...
def get_download_link(wb, filename="products_updated.xlsx"):
    ...
    return f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="{filename}" class="stButton>button" style="display:block; text-align:center; background:#10b981; color:white; padding:15px; border-radius:10px; text-decoration:none; font-size:20px; font-weight:bold; margin-top:20px;">📥 تحميل الملف المكتمل</a>'
...
with st.sidebar:
    ...
    store_name = st.text_input("اسم المتجر", "لي غابريال")
    ...
    if st.button("🚀 بدء المعالجة الشاملة الآن", type="primary"):
        if not active_keys: st.error("❌ أدخل مفتاح API واحد على الأقل.")
            ...
            try:
                ...
                st.warning("تم إيقاف العملية، لكن ما تم إنجازه محفوظ بأمان!")
            finally:
                loop.close()
                dl_st.markdown(get_download_link(wb, "Salla_Products_3200_Done.xlsx"), unsafe_allow_html=True)
                st.balloons()
