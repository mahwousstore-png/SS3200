...
# ══════════════════════════════════════════════════════════════
#  مولّد أوصاف عطور احترافي (نسخة المطابقة مع فلتر الأخطاء الذكي)
#  متوافق مع سلة 100% | بدون فراغات أسطر
# ══════════════════════════════════════════════════════════════
...
def fetch_notes(name: str, api_key: str, model: str, store_name: str, provider: str) -> dict | None:
    """جلب معلومات العطر بناءً على المزود وتوسيع النص لأكثر من 2000 حرف مع فلتر قوي للأخطاء"""
...
أرجع النتيجة بصيغة JSON فقط وبدون أي نص إضافي:
{
  ...
  "top_notes": "وصف طويل للنوتات العليا ومكوناتها وتأثيرها على الحواس",
  "heart_notes": "وصف طويل للنوتات الوسطى وكيف تتناغم",
  ...
  "why_choose_1": "سبب أول قوي لاختيار العطر مع شرح تفصيلي.",
  "why_choose_2": "سبب ثاني مع تفصيل الميزات.",
  "why_choose_3": "سبب ثالث مع تفصيل الأوقات المناسبة.",
  "faq_1_q": "سؤال شائع أول (مثال: هل العطر مناسب للاستخدام اليومي؟)",
  "faq_1_a": "إجابة مفصلة للسؤال الأول.",
  "faq_2_q": "سؤال شائع ثاني",
  "faq_2_a": "إجابة مفصلة للسؤال الثاني.",
  "faq_3_q": "سؤال شائع ثالث",
  "faq_3_a": "إجابة مفصلة للسؤال الثالث.",
  "closing_paragraph": "خاتمة طويلة وجذابة تؤكد على روعة العطر وتدفع العميل للشراء بثقة."
}"""

    user_msg = f'اكتب وصفاً احترافياً مطولاً (أكثر من 2000 حرف) للمنتج: "{name}" لمتجر "{store_name}". استخدم بيانات دقيقة من Fragrantica.'
    ...
    try:
        ...
        # الفلتر الذكي لاستخراج كود الـ JSON فقط وتجاهل أي كلام إضافي من الذكاء الاصطناعي
        match = re.search(r'\{.*\}', text, re.DOTALL)
        if match:
            parsed_data = json.loads(match.group(0))
            # التأكد من أن النتيجة عبارة عن Dictionary
            if isinstance(parsed_data, dict):
                return parsed_data
        
        return None
    except Exception as e:
        # في حال وجود أي خطأ، سيرجع القيمة None ليتم تخطي المنتج دون إيقاف البرنامج
        return None
...
def process_file(uploaded, active_keys, model, store_name, store_link, process_all, sleep_time, bar, status):
    ...
    for idx, (row_i, pname) in enumerate(tasks):
        ...
        # التأكد من أن البيانات موجودة وأنها Dictionary
...
st.info("💡 تم تفعيل فلتر الأخطاء الذكي لضمان عدم توقف الكود أبداً حتى لو أخطأ الذكاء الاصطناعي في تنسيق الإجابة.")
...
if uploaded:
    ...
    if st.button("🚀 ابدأ المعالجة الآن", type="primary"):
        if not active_keys:
            ...
            if success > 0:
                st.success(f"✅ تمت العملية بنجاح! تم وصف {success} منتج بشكل صحيح.")
                st.download_button(
                    "📥 تحميل الملف الجاهز",
                    data=buf,
                    file_name="products_updated_bulletproof.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True
                )
            else:
                st.error("❌ فشلت المعالجة. تأكد من المفاتيح أو جرب زيادة وقت الانتظار قليلاً.")
